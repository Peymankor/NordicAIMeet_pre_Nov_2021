...
class EnergyStoragePolicy():
    ...
    def bellman_policy(self, time,state, bellman_model):
        ...
        #print("***********************Starting to Solve tree from left to right**************************")
        price = state.price
        energy = state.energy_amount
        
        #print("The price value :{} and energy value: {}".format(price, energy))
        maxValue = -np.inf
        maxDec = None
        for d in self.model.possible_decisions:
            ...
            for w in bellman_model.discrete_price_changes:
                ...
                #print("The contribution is {} and the next state is:{} the V+1 values are {}".format(contribution,next_state,next_v))
                sum_w += f * next_v
                #sum_w += 0.5 * next_v
                w_index += 1
            # print("w_index={}".format(w_index))
            v = contribution + sum_w
            #print("The contribution is {} and the V+1 values are {}".format(contribution, sum_w))

            if (v>maxValue):
                maxValue=v
                maxDec=d
        #print("maxDec.{}".format(maxDec))
        return maxDec
    ...
    def run_policy(self, policy_info, policy, stop_time):
        ...
        buy_list = []
        sell_list = []
        battery_storage_values = []
        while time != model_copy.init_args['T']:
            

            decision = getattr(self,policy)(time,model_copy.state, policy_info)
            #print("decision decision.{}".format(decision))
            modelcopy = model_copy.state
            #print("price {}".format(modelcopy.price))
            ...
            #print("state energy amount.{}".format(model_copy.state.energy_amount))
            battery_storage_values.append(model_copy.state.energy_amount)
            nTrades['buy'] += x.buy
            nTrades['sell'] += x.sell
        ...
        #return contribution
        return {"contribution": contribution, "battrey_storage": battery_storage_values}
